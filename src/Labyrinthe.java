...
public class Labyrinthe implements Serializable {
    ...
    public String toString() {
        ...
        for (int y = 0; y < ny; y++) {
            ...
            for (int x = 0; x < nx; x++) {
                ...
                else if (cell.getElement() instanceof Mur) {
                    sb.append("###|");
                } else if (cell.getElement() instanceof Cactus) {
                    sb.append("/*/|");
                } else if (cell.getElement() instanceof marguerite) {
                    sb.append("!!!|");
                }
                else if (cell.getElement() == null) {
                    sb.append("tub");
                }
                else {
                    sb.append("   |");
    ...
    public boolean CasserMur(int x /*droite*/, int y/*hauteur*/) {
        ...
        if (this.cellules.get(x).get(y).getElement() instanceof Mur) {
            this.cellules.get(x).get(y).setElement(new Herbe());
            return true;
        }
    ...
    public boolean PoserMargueurite(int x, int y) {
        ...
        this.cellules.get(x).get(y).setElement(new marguerite());
        return true;
    }
    ...
    public boolean PoserCactus(int x, int y) {
        ...
        this.cellules.get(x).get(y).setElement(new Cactus());
        return true;
    }
    ...
    public boolean PoserMur(int x, int y) {
        ...
        this.cellules.get(x).get(y).setElement(new Mur());

        return true;
    }

    public boolean DefinirSortie(int x, int y) {
        if (this.sortie == true) {
        ...
        if (cellules.get(x).get(y).getElement() instanceof Vegetal) {
    ...
    public ArrayList<Cellule> getVoisins(Cellule cellule ) {
        ...
        if (this.cellules.get(cellule.getX() - 1).get(cellule.getY()).getElement() instanceof Vegetal) {
        ...
        if (this.cellules.get(cellule.getX()).get(cellule.getY() + 1).getElement() instanceof Vegetal) {
